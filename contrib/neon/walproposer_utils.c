...
/* START cloned file-level variables and functions from walsender.c */
...
StartProposerReplication(StartReplicationCmd *cmd)
{
	 ...
	 */
	if (GetFlushRecPtr() < cmd->startpoint)
	{
		ereport(ERROR,
				(errmsg("requested starting point %X/%X is ahead of the WAL flush position of this server %X/%X",
						LSN_FORMAT_ARGS(cmd->startpoint),
						LSN_FORMAT_ARGS(GetFlushRecPtr()))));
	}
...
/*
 * Notify walproposer about latest flushed LSN.
 */
static void
XLogSendPhysical(void)
{
	...
	WalProposerBroadcast(sentPtr, SendRqstPtr);
	sentPtr = SendRqstPtr;
