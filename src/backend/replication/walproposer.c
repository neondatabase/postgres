...
static void WalProposerInit(XLogRecPtr flushRecPtr, uint64 systemId, TimeLineID tli);
...
WalProposerMain(Datum main_arg)
{
	...
	WalProposerInit(GetFlushRecPtr(NULL), GetSystemIdentifier(), tli);
...
WalProposerSync(int argc, char *argv[])
{
	struct stat stat_buf;
	 // FIXME Write a comment, why this hardcoded value is safe
	TimeLineID tli = 1;
...
static void
WalProposerInit(XLogRecPtr flushRecPtr, uint64 systemId, TimeLineID tli)
{
	char	   *host;
	char	   *sep;
	char	   *port;
