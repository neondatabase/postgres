...
static void
hmac_sha256(uint8 *result, const uint8 *key, int keylen, const uint8 *msg, int msglen)
{
...
#define HMAC_OPAD 0x5C
	...
	if (pg_cryptohash_init(sha256ctx) < 0 ||
		...
		pg_cryptohash_final(sha256ctx, result, PG_SHA256_DIGEST_LENGTH) < 0)
	{
		pg_cryptohash_free(sha256ctx);
		pg_fatal("cryptohash failure");
	}
				  ...
				  const char *secret)
{
	...
	hmac_sha256(hmacbuf, (uint8 *) awssecret, strlen(awssecret), (const uint8 *) today_buf, today_len); /* DateKey */
	hmac_sha256(hmacbuf, hmacbuf, PG_SHA256_DIGEST_LENGTH, (const uint8 *) region, strlen(region)); /* DateRegionKey */
	hmac_sha256(hmacbuf, hmacbuf, PG_SHA256_DIGEST_LENGTH, (const uint8 *) service, strlen(service)); /* DateRegionServiceKey */
	hmac_sha256(hmacbuf, hmacbuf, PG_SHA256_DIGEST_LENGTH, (const uint8 *) "aws4_request", strlen("aws4_request")); /* SigningKey */
...
construct_string_to_sign(pg_time_t now, const char *method, const char *host, const char *region, const char *path, const char *query_string, const char *bodyhash)
{
	...
	appendStringInfo(&canonical_request, "host;x-amz-date\n");
	/* PAYLOAD */
