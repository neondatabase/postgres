...
select array_agg(distinct ar order by ar desc)
  from (select array[i / 2] from scaled_series(1,10) a(i)) b(ar);
select array_agg(ar)
  from (select array_agg(array[i, i+1, i-1])
        from scaled_series(1,2) a(i)) b(ar);
select array_agg(array[i+1.2, i+1.3, i+1.4]) from scaled_series(1,3) g(i);
select array_agg(array['Hello', i::text]) from scaled_series(9,11) g(i);
select array_agg(array[i, nullif(i, 3), i+1]) from scaled_series(1,4) g(i);
-- errors
select array_agg('{}'::int[]) from scaled_series(1,2);
select array_agg(null::int[]) from scaled_series(1,2);
select array_agg(ar)
...
select array(select array[i,i/2] from scaled_series(1,5) i);
select array(select array['Hello', i::text] from scaled_series(9,11) i);

-- int2vector and oidvector should be treated as scalar types for this purpose
select pg_typeof(array(select '11 22 33'::int2vector from scaled_series(1,5)));
select array(select '11 22 33'::int2vector from scaled_series(1,5));
select unnest(array(select '11 22 33'::int2vector from scaled_series(1,5)));
select pg_typeof(array(select '11 22 33'::oidvector from scaled_series(1,5)));
select array(select '11 22 33'::oidvector from scaled_series(1,5));
select unnest(array(select '11 22 33'::oidvector from scaled_series(1,5)));
...
insert into src
  select string_agg(random()::text,'') from scaled_series(1,10000);
