...
SELECT array_to_json(array_agg(q),false) from (select x as b, x * 2 as c from generate_series(1,3000000) x) q;
...
SELECT array_to_json(array_agg(x),false) from generate_series(5,10000) x;
...
SELECT row_to_json(row((select array_agg(x) as d from generate_series(5,10000) x)),false);
