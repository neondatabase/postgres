...
SELECT array_to_json(array_agg(q),false) from (select x as b, x * 2 as c from scaled_series(1,3) x) q;
SELECT array_to_json(array_agg(q),true) from (select x as b, x * 2 as c from scaled_series(1,3) x) q;
...
SELECT array_to_json(array_agg(x),false) from scaled_series(5,10) x;
...
SELECT row_to_json(row((select array_agg(x) as d from scaled_series(5,10) x)),false);
...
select count(*) from
    (select json_object_keys(json_object(array_agg(g)))
     from (select unnest(array['f'||n,n::text])as g
           from scaled_series(1,300) as n) x ) y;
...
select json_object_agg_unique(mod(i,100), i) from scaled_series(0, 199) i;
