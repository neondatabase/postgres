...
SELECT
  (SELECT n
     FROM (VALUES (1)) AS x,
          (SELECT n FROM scaled_series(1,10) AS n
             ORDER BY n LIMIT 1 OFFSET s-1) AS y) AS z
  FROM scaled_series(1,10) AS s;
...
explain (verbose, costs off)
select unique1, unique2, scaled_series(1,10)
...
explain (verbose, costs off)
select generate_series(0,2) as s1, scaled_series((random()*.1)::int,2) as s2;
