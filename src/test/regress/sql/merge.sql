...
-- index plans
INSERT INTO target SELECT generate_series(1000,10000000), 0;
...
CREATE TABLE ex_msource (a int, b int)
  WITH (autovacuum_enabled=off);
INSERT INTO ex_mtarget SELECT i, i*10 FROM generate_series(1,1000000,2) i;
INSERT INTO ex_msource SELECT i, i*10 FROM generate_series(1,1000000,1) i;
...
INSERT INTO pa_source SELECT id, id * 10  FROM generate_series(1,1400000) AS id;
-- insert a few rows in the target table (odd numbered tid)
INSERT INTO pa_target SELECT id, id * 100, 'initial' FROM generate_series(1,140000,2) AS id;
...
INSERT INTO pa_source SELECT id, id * 10  FROM generate_series(1,1400000) AS id;
-- insert a few rows in the target table (odd numbered tid)
INSERT INTO pa_target SELECT '2017-01-31', id, id * 100, 'initial' FROM generate_series(1,90000,3) AS id;
INSERT INTO pa_target SELECT '2017-02-28', id, id * 100, 'initial' FROM generate_series(2,90000,3) AS id;
...
USING generate_series(1,1000000,1) AS id
ON t.a = id
WHEN MATCHED THEN
	UPDATE SET b = b + id
WHEN NOT MATCHED THEN
...
MERGE INTO fs_target t
USING generate_series(1,1000000,2) AS id
ON t.a = id
WHEN MATCHED THEN
