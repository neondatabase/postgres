...
INSERT INTO current_check_1 SELECT i, 'p' || i FROM generate_series(1,900) i;
...
rollback;
begin;
explain (costs off) declare c2 cursor for select generate_series(1,300) as g;
explain (costs off) declare c2 scroll cursor for select generate_series(1,300) as g;
declare c2 scroll cursor for select generate_series(1,300) as g;
fetch all in c2;
