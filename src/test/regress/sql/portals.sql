...
INSERT INTO current_check_1 SELECT i, 'p' || i FROM generate_series(1,9000000) i;
INSERT INTO current_check_2 SELECT i, 'P' || i FROM generate_series(10,190) i;
...
rollback;
begin;
explain (costs off) declare c2 cursor for select generate_series(1,30000) as g;
explain (costs off) declare c2 scroll cursor for select generate_series(1,30000) as g;
declare c2 scroll cursor for select generate_series(1,30000) as g;
fetch all in c2;
...
insert into toasted_data
  select array_agg(i) from generate_series(12345678, 12345678 + 10000) i;
