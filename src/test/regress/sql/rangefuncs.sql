...
select row_to_json(s.*) from scaled_series(11,14) with ordinality s;
-- ordinality vs. views
...
select * from rows from(unnest(array[10,20],array['foo','bar']), scaled_series(101,102)) with ordinality as z(a,b,c,ord);
...
select definition from pg_views where viewname='vw_ord';
drop view vw_ord;
create temporary view vw_ord as select * from rows from(unnest(array[10,20],array['foo','bar']), scaled_series(1,2)) as z(a,b,c);
select * from vw_ord;
select definition from pg_views where viewname='vw_ord';
drop view vw_ord;
...
begin;
declare rf_cur scroll cursor for select * from rows from(generate_series(1,5),scaled_series(1,2)) with ordinality as g(i,j,o);
...
CREATE FUNCTION rngfunc_sql(int,int) RETURNS setof rngfunc_rescan_t AS 'SELECT i, nextval(''rngfunc_rescan_seq1'') FROM scaled_series($1,$2) i;' LANGUAGE SQL;
...
SELECT * FROM (VALUES (1),(2),(3)) v(r) LEFT JOIN scaled_series(11,13) f(i) ON (r+i)<100;
SELECT * FROM (VALUES (1),(2),(3)) v(r) LEFT JOIN scaled_series(11,13) WITH ORDINALITY AS f(i,o) ON (r+i)<100;
...
SELECT setval('rngfunc_rescan_seq1',1,false),setval('rngfunc_rescan_seq2',1,false);
SELECT * FROM generate_series(1,2) r1, scaled_series(r1,3) r2, ROWS FROM( rngfunc_sql(10+r1,13), rngfunc_mat(10+r2,13) );

SELECT * FROM (VALUES (1),(2),(3)) v(r), scaled_series(10+r,20-r) f(i);
SELECT * FROM (VALUES (1),(2),(3)) v(r), scaled_series(10+r,20-r) WITH ORDINALITY AS f(i,o);
...
SELECT * FROM (VALUES (1),(2),(3)) v1(r1),
              LATERAL (SELECT r1, * FROM (VALUES (10),(20),(30)) v2(r2)
                                         LEFT JOIN scaled_series(21,23) f(i) ON ((r2+i)<100) OFFSET 0) s1;
SELECT * FROM (VALUES (1),(2),(3)) v1(r1),
              LATERAL (SELECT r1, * FROM (VALUES (10),(20),(30)) v2(r2)
                                         LEFT JOIN scaled_series(20+r1,23) f(i) ON ((r2+i)<100) OFFSET 0) s1;
SELECT * FROM (VALUES (1),(2),(3)) v1(r1),
              LATERAL (SELECT r1, * FROM (VALUES (10),(20),(30)) v2(r2)
                                         LEFT JOIN scaled_series(r2,r2+3) f(i) ON ((r2+i)<100) OFFSET 0) s1;
SELECT * FROM (VALUES (1),(2),(3)) v1(r1),
              LATERAL (SELECT r1, * FROM (VALUES (10),(20),(30)) v2(r2)
                                         LEFT JOIN scaled_series(r1,2+r2/5) f(i) ON ((r2+i)<100) OFFSET 0) s1;
...
FROM (VALUES (1),(2)) v1(r1)
    LEFT JOIN LATERAL (
        SELECT *
        FROM scaled_series(1, v1.r1) AS gs1
        LEFT JOIN LATERAL (
            SELECT *
            FROM scaled_series(1, gs1) AS gs2
            LEFT JOIN scaled_series(1, gs2) AS gs3 ON TRUE
        ) AS ss1 ON TRUE
        FULL JOIN scaled_series(1, v1.r1) AS gs4 ON FALSE
    ) AS ss0 ON TRUE;
...
RETURNS TABLE(a int)
AS $$ SELECT a FROM scaled_series(1,5) a(a) $$ LANGUAGE sql;
SELECT * FROM rngfunc();
...
AS $$ SELECT a, b
         FROM scaled_series(1,$1) a(a),
              scaled_series(1,$1) b(b) $$ LANGUAGE sql;
...
create function rngfunc1(n integer, out a text, out b text)
  ...
  language sql
  as $$ select 'foo ' || i, 'bar ' || i from scaled_series(1,$1) i $$;
...
SELECT * FROM ROWS FROM(scaled_series(10,11), get_users()) WITH ORDINALITY;
SELECT * FROM ROWS FROM(get_users(), scaled_series(10,11)) WITH ORDINALITY;
