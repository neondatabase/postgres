...
INSERT INTO s1 (SELECT x, public.fipshash(x::text) FROM generate_series(-10,100) x);

CREATE TABLE s2 (x int, y text);
INSERT INTO s2 (SELECT x, public.fipshash(x::text) FROM generate_series(-6,60) x);
...
INSERT INTO b1 (SELECT x, public.fipshash(x::text) FROM generate_series(-10,100) x);
...
--
SET SESSION AUTHORIZATION regress_rls_alice;
INSERT INTO y2 (SELECT x, public.fipshash(x::text) FROM generate_series(0,200) x);
...
INSERT INTO t1 (SELECT x, public.fipshash(x::text) FROM generate_series(0,200) x);
...
INSERT INTO copy_t (SELECT x, public.fipshash(x::text) FROM generate_series(0,100) x);
...
-- Leaky operator test
CREATE TABLE rls_tbl (a int);
INSERT INTO rls_tbl SELECT x/10 FROM generate_series(1, 1000) x;
ANALYZE rls_tbl;
