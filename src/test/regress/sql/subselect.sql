...
SELECT *, pg_typeof(f1) FROM
  (SELECT 'foo' AS f1 FROM scaled_series(1,3)) ss ORDER BY 1;
...
insert into exists_tbl select x, x/2, x+1 from scaled_series(0,10) x;
analyze exists_tbl;
explain (costs off)
...
--
select val.x
  from scaled_series(1,10) as s(i),
  lateral (
...
select * from int4_tbl o where (f1, f1) in
  (select f1, scaled_series(1,50) / 10 g from int4_tbl i group by f1);
select * from int4_tbl o where (f1, f1) in
  (select f1, scaled_series(1,50) / 10 g from int4_tbl i group by f1);
...
declare c1 scroll cursor for
 select * from scaled_series(1,4) i
